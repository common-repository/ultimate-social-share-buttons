...
if(get_option('icon-inexcerpt') == 'yes'){
    add_filter('the_excerpt', 'ussb_social_share_icons'); //the_excerpt content
}
add_filter('the_content', 'ussb_social_share_icons'); //the_content post content
function ussb_social_share_icons($ssb_content){
        ...
        $ssb_output = '<div class="ssb_social-icons"> <ul class="social-menu">';
                foreach ($ssb_icons as $ssb_platform => $ssb_data) {
                    ...
                    if (get_option($option_key)) {
                        $ssb_output .= '<li> <a href="' . esc_url($ssb_data['url']) . '" target="_blank"><img src="' . esc_url(USSB_PLUGIN_URL . 'images/' . $ssb_data['icon']) . '" alt="' . $ssb_platform . '" title="Share in '. $ssb_platform .'"></a> </li>';
                    }
                }
        $ssb_output .= '</ul> </div>';
...
add_action('wp_head', 'ussb_styleadd_dynamically');
function ussb_styleadd_dynamically(){ ?> 
